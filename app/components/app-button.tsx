...
interface ButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  ...
  variant?: "primary" | "secondary" | "flat"; // Optional variant prop for styling
...
const AppButton: React.FC<ButtonProps> = ({
  ...
  const variantClasses =
    variant === "primary"
      ...
      : variant === "secondary"
      ? "border-2 border-blue-500 text-blue-500 hover:bg-blue-100 focus:ring-blue-300 shadow-md"
      : "bg-transparent text-blue-500 hover:bg-blue-100 focus:ring-blue-300 shadow-md"; // Enhanced hover/focus effects

  return (
    <button
      className={`${baseClasses} ${variantClasses} ${className || "w-full"} ${
        endIcon ? "flex justify-between" : ""
      } font-bold ${disabled ? "opacity-50 cursor-not-allowed" : ""}`} // Combine classes and adjust layout for endIcon
      onClick={onClick}
      disabled={disabled}
    >
